...
/**
* @brief  usbd_scsi_read
*         Process Read10/12 command
* @param  params: Command parameters
* @retval status
*/
static int usbd_scsi_read(usbd_msc_dev_t  *cdev, u8 *params)
...
/**
* @brief  usbd_scsi_write
*         Process Write10/12 command
* @param  params: Command parameters
* @retval status
*/
...
static int usbd_scsi_write(usbd_msc_dev_t  *cdev, u8 *params)
{
	...
	if (cdev->bot_state == USBD_MSC_IDLE) { /* Idle */
		/* case 3, 8 : Hi <> Do */
		if ((cdev->cbw.DataTransferLength) == 0 || (cdev->cbw.Flags & 0x80U) == 0x80U) {
			usbd_scsi_sense_code(cdev, ILLEGAL_REQUEST, INVALID_CDB);
			cdev->phase_error = 1;
			return -1;
		}
